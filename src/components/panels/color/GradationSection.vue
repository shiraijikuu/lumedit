<template>
  <div class="grad">
    ...
    <template v-if="store.params.gradation.enabled">
      ...
      <SliderRow :label="t('gradation.width')" :model-value="store.params.gradation.w" :min="0.05" :max="3" :step="0.01" :decimals="2" @update:model-value="set((g) => (g.w = $event))" @scrub-start="store.mutate(() => {}, true)" @scrub-end="store.endScrub()" @reset="set((g) => (g.w = 0.6))" />
      <SliderRow v-if="store.params.gradation.type === 'radial'" :label="t('gradation.height')" :model-value="store.params.gradation.h" :min="0.05" :max="3" :step="0.01" :decimals="2" @update:model-value="set((g) => (g.h = $event))" @scrub-start="store.mutate(() => {}, true)" @scrub-end="store.endScrub()" @reset="set((g) => (g.h = 0.6))" />
